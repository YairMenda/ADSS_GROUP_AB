...
public class StorageFacade {
    ...
    public boolean sellItem(String storageName, int productId) throws Exception
    {
        Product p = getProduct(storageName, productId);
        if(p == null)
            throw new Exception("Product doesnt exist");
        //need to add prod doesnt exists
        boolean res = this.storages.get(storageName).sellItem(productId);
        if(!res)
            throw new Exception("No available products-items");
        return true;
    }
